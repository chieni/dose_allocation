...
import pandas as pd
from helpers import alpha_func, get_expected_improvement, get_ucb, get_toxicity, gradient_ascent_update
...
def run_C3T_with_gradient(T, B, S, K, pats, arr_rate, tox_thre, eff_thre, p_true, q_true, opt_ind, dose_labels, no_skip=False):
    '''
    Input
    T        : time-horizon
    B        : budget
    ...
    I = np.ones(T, dtype=int) * -1 # allocated doses
    X = np.zeros(T) # efficacy
    Y = np.zeros(T) # toxicity
    H = pats
    Z = np.zeros(T)
...
def run_C3T_with_gradient_temp(T, B, S, K, pats, arr_rate, tox_thre, eff_thre, p_true, q_true, opt_ind, dose_labels):
    '''
    No budget version
    ...
    while t < T and current_budget < B:
        ...
        # print(get_toxicity(dose_labels[curr_s, :], new_alpha), p_hat[curr_s, :])
        # ak_hat[curr_s, I[t]] = np.log(p_hat[curr_s, I[t]]) / \
        #                         np.log((np.tanh(dose_labels[curr_s, I[t]]) + 1.) / 2.)
        # if ak_hat[curr_s, I[t]] > a_max:
        #     ak_hat[curr_s, I[t]] = a_max
    ...
    rec, cum_s, typeI, typeII, q_mse, rec_err, a_hat_fin = finalize_results_with_gradient(T, t, K, S, X, H, n_choose, current_a_hat, dose_labels,
                     tox_thre, eff_thre, q_mse, q_bar, p_true, opt_ind)
    return rec, cum_eff, cum_tox, cum_s, typeI, typeII, q_mse, rec_err, a_hat_fin, p_hat



def finalize_results_with_gradient(T, t, K, S, X, H, n_choose, a_hat, dose_labels,
                     tox_thre, eff_thre, q_mse, q_bar, p_true, opt_ind,
                     uses_context=True):
    # Recommendation and observe results
    rec = np.zeros((S, K+1))
