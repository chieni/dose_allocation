    ...
    def __init__(self, num_latents, num_tasks, inducing_points):
        ...
        self.covar_module = gpytorch.kernels.ScaleKernel(
            gpytorch.kernels.RBFKernel(batch_shape=torch.Size([num_latents]),
                                       lengthscale_prior=gpytorch.priors.LogNormalPrior(0.4, 0.1)) + \
                gpytorch.kernels.LinearKernel(batch_shape=torch.Size([num_latents]),
                                              variance_prior=gpytorch.priors.SmoothedBoxPrior(0.1, 1.0),
                                              variance_constraint=gpytorch.constraints.Positive()),
            batch_shape=torch.Size([num_latents]),
            outputscale_prior=gpytorch.priors.LogNormalPrior(0.4, 0.1)
        )
