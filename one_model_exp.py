...
from one_gp import ClassificationRunner, ClassificationGPModel
...
class DoseExperimentMetrics:
    def __init__(self, dose_scenario, subgroup_indices, selected_doses, tox_outcomes, eff_outcomes,
        ...
        grouped_metrics_frame['final_dose_diff'] = np.concatenate([final_dose_diff, [sum(final_dose_diff) / self.num_subgroups]])
        grouped_metrics_frame['final_dose_diff_abs'] = np.concatenate([final_dose_diff_abs, [sum(final_dose_diff_abs) / self.num_subgroups]])
        ...
        self.final_dose_rec = pd.DataFrame({'final_dose_rec': final_selected_doses,
                                            'final_dose_diff': final_dose_diff,
                                            'final_dose_diff_abs': final_dose_diff_abs})
...
def get_model_predictions(runner, num_subgroups, x_test, num_confidence_samples, use_gpu):
    ...
    post_latents, _ = runner.predict(x_test)
...
def select_final_dose(num_subgroups, num_doses, dose_labels, x_test, max_doses,
                      ...
                      tox_thre, eff_thre, p_param, final_beta_param):
...
def online_dose_finding(filepath, dose_scenario, patient_scenario,
                        ...
                        use_utility, use_lcb_init, use_lcb_exp, set_lmc, use_gpu):
    ...
    while timestep < num_samples:
        ...
        # Train model
        tox_runner = ClassificationRunner(dose_labels, tox_mean_init,
                                         tox_lengthscale_init)
        tox_runner.train(x_train, y_tox_train, num_epochs, learning_rate)

        eff_runner = ClassificationRunner(dose_labels, eff_mean_init,
                                          eff_lengthscale_init)
        eff_runner.train(x_train, y_eff_train, num_epochs, learning_rate)
...
if __name__ == "__main__":
    ...
    if run_one:
        ...
        online_dose_finding_trials(filepath, num_trials, dose_scenario,
                                ...
                                use_lcb_init, use_lcb_exp, set_lmc, use_gpu)
