...
class BernoulliBandit():
    ...
    def select_arm(self, arm_idx, group_idx):
        '''
...
class GroupedBernoulliBandit():
    def __init__(self, num_arms, reward_dict):
        self.num_arms = num_arms
        self.arm_indices = [idx for idx in range(self.num_arms)]
        self.reward_dict = reward_dict
        self.best_reward_prob_dict = {key: max(probs) for (key, probs) in self.reward_dict.items()}
        self.reward_variances = None
    ...
    def select_arm(self, arm_idx, group_idx):
        ...
        Binary reward based on self.reward_probs set during initialization.
        '''
        if np.random.random() < self.reward_dict[group_idx][arm_idx]:
            return 1
        return 0
