...
def select_dose_confidence_and_increasing(num_doses, max_dose, tox_mean, tox_upper,
    ...
    return violation_flag, selected_dose, tox_acqui, dose_set_mask
...
def select_dose(num_doses, max_dose, tox_mean, tox_upper,
    ...
    if dose_set_mask.sum() == 0:
        selected_dose = 0
        violation_flag = True
    return violation_flag, selected_dose, tox_acqui, eff_ei
    ...
    while timestep < num_samples:
        ...
        for subgroup_idx in range(patient_scenario.num_subgroups):
            if timestep <= sampling_timesteps:
                violation_flag, selected_dose_by_subgroup[subgroup_idx], tox_acqui_funcs[subgroup_idx, :], dose_set_mask[subgroup_idx, :] \
                ...
                violation_flag, selected_dose_by_subgroup[subgroup_idx], tox_acqui_funcs[subgroup_idx, :],\
        ...
        if num_violations >= max_num_violations:
            # Early stopping
            break
        timestep += cohort_size
